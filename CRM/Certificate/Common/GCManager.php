...
/**
 * Adaptive Garbage Collection Manager for Certificate Extension
 *
 * Implements industry best practices for memory management during certificate operations
 */
    ...
    if ($shouldCollect && function_exists('gc_collect_cycles')) {
      ...
      if ($cycles > 0 || $memoryFreed > (5 * 1024 * 1024)) {
        \Civi::log()->debug(sprintf(
          'Certificate GC (%s): %d cycles, %.2fms, %s freed, memory: %s, interval: %d',
          $reason,
          $cycles,
          $duration,
